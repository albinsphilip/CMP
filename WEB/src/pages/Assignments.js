    ...
    useEffect(() => {
      ...
      const fetchFile = async () => {
        ...
        for (let i = 0; i < numberOfFiles; i++) {
          try {
            const response = await axios(`/getFile/${sub}/${i}`, {
              method: 'GET',
  ...
  <li className={isDesktop ? "attcont3" : "attcont2"} key={item.date}>
    ...
    <h5>Uploaded date: {item.upload}</h5>
      ...
      <div>  <a className="pdf-button" key={index} href={file.url} target="_blank" rel="noopener noreferrer">
        {file.sub}  Open {file.type} {new Date(file.creationTime).toLocaleDateString() + ' '+new Date(file.creationTime).toLocaleTimeString() }
        </a></div>
