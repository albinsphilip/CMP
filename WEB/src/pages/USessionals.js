...
function USessionals() {
    ...
    const students =(data)=>{
        if (!data) return [];
        let result = data.map(item => ({Rollno:item.classroll, student: item.name, roll: item.rollno, class: item.class }));
        return result;
    ...
    function handleatt(event){
        ...
        axios.post("/updatemark", {series,selectedValues }).then((response) => {
    ...
    function handlemarksheet(event){
       ...
       if(jsonData){
        axios.post("/updatemark", {jsonData }).then((response) => {
        if(response.data.message==='success'){
         setCell(true);
        }
